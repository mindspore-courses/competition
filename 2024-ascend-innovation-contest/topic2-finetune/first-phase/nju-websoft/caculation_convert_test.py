...
def mass_computation_process(sample):
    ...
    process = str(den) + ' * ' + str(vol) + ' = ' + str(result) + ' 克'
    return process
...
def area_computation_process(sample):
    ...
    if abs(result-int(solu)) >= 1e-2:
        raise CustomError('mass computation not equal', '\n'.join(
            [str(sample), str(result), solu]))
    process = "面积为 " + str(length) + ' * ' + str(width) + \
        ' = ' + str(result) + ' 平方厘米'
    return process
...
if __name__ == "__main__":
    ...
    train_cn, train_eg = read_data('train-v2.json')
    train_data = excute_classification(train_cn)
    train_data['eg'] = train_eg

    test_data = {}
    for k, lis in tqdm(classed_data.items()):
        problem_lis = {d['problem']: d for d in train_data[k]}
        test_data[k] = [d for d in lis if d['problem'] not in problem_lis]

    sampled_and_shuffled = sample_and_shuffle(test_data, 2000)
    with open('test-v2.json', 'w', encoding='utf-8') as f:
