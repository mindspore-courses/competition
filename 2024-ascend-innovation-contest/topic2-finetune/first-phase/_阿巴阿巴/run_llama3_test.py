...
def main(task='text_generation',
    ...
    #prompt = '你正在进行LoRA微调以加强你的数学处理能力，接下来，你会收到一个数学问题，你必须精准的给出这个数学问题的答案。\n'
    prompt =''
    with open(input_dir, 'r', encoding='utf-8') as file:
        ...
        for line in file:
            ...
            #添加提示词
            pro_list = f"{prompt}{line['problem']}"
            print(pro_list)
    ...
    if run_mode == 'train':
        ...
        #    for j in range(len(result[i]['text_generation_text'])):
        #        result[i]['text_generation_text'][j] = result[i]['text_generation_text'][j].replace(prompt,'')
        logger.info(result)
