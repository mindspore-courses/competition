...
class CausalLanguageModelingTrainer(BaseTrainer):
    ...
    def generate_evaluate(self,
        ...
        save_path = '/home/ma-user/work/infer_result/'

        if not os.path.isdir(save_path):
            logger.info("Create %s to cache logits" % save_path)
        ...
        # 222222222222222222222222222222222222222222222
        name_npy = 'result_infer.npy'
        fpath = name_npy
        if not os.path.exists(fpath):
            with open(fpath, 'wb') as f:
                np.save(f, result_all)
        else:
            logger.info("Cache logits path: %s already exists, skipped" % fpath)
        # 222222222222222222222222222222222222222222222
