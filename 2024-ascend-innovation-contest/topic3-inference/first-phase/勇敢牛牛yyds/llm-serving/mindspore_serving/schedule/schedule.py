...
class Schedule:
    ...
    def __init__(self, config: ServingConfig):
        self.swapped_request_queue: Deque[EntryMetaData] = Deque([])
        ...
        ########### add for _passed_delay
        self.prev_prompt = False ### 用于判断之前是否发生过从waiting队列调度到running序列
        self.prev_time = 0.0
    ...
    def _determine_batch_size(self):
        ...
        if bf_batch >= af_batch:
            ...
            for i in range(bf_batch, af_batch):
                entry_meta_data = EntryMetaData(arrival_time = time.time(),
                                                page_attention=self.config.model_config.page_attention,
    ...
    def _insert_new_prompt_to_batch_pa_swapped(self, index):
        # logging.debug('add a new request into batching list')
        # data = self.waiting_request_queue.get_nowait()
        data = self.swapped_request_queue.popleft()
        if data.entry_data.get_prompt_len() + data.entry_data.get_max_token_len() >= self.max_input_len:
            data.get_entry_data().set_status(EntryStatus.INPUT_OUTOFRANGE)
        else:
            data.get_entry_data().set_status(EntryStatus.RUNNING)
    ...
    # 如果有空槽位，尝试替代一条新请求：
    def try_substitute_entry_swapped(self):
        ### 获取batch中所有请求的状态
        checkout_list = self.checkout_entry()
        is_invalid_index_list = []
        for index, is_invalid in enumerate(checkout_list):
        ...
        logging.debug("trying to substitute old entry at index: %s", index_to_substitute)
        # 如果新entry需要的block数量，小于can_substitute entry的block数量 + mem pool全局剩余block数量
        new_entry = self.swapped_request_queue[0]
        if new_entry.cache_engine.try_use_budget(new_entry.get_entry_data().get_len()):
            self._insert_new_prompt_to_batch_pa_swapped(index_to_substitute)
            return True
        logging.debug("failed inserting to existing entry")
        # 如果空间不足，那么连第一条waiting的请求就无法替换，直接退出
        return False
    ...
    def can_predict_current_batch(self):
        ...
        for index, is_invalid in enumerate(checkout_list):
            ...
            ###对当前请求，是否有足够的显存空间支持，如果对所有的prompt都有空间，那么返回true
            if entry.is_prompt:
    ...
    def try_initialize_paddings_pa(self):
        ...
        ### 采用 end_token填充
        for index in range(len(self.running_request_list), self.batch_size):
            padding_entry = EntryMetaData(arrival_time = time.time(),
                                          page_attention=self.config.model_config.page_attention,
                                          request_id=PADDING_REQUEST_ID,
    ...
    def try_swap_valid_entries(self):
        ...
        self.swapped_request_queue.extend(entry_to_swap) ### 插入swap队列
        # 用padding替代
    ...
    def _passed_delay(self, now: float) -> bool:
        ...
        # ### 这样做也可以避免self.last_prompt_latency变成一个很大的值，不然passed_delay会一直为false
        self.prev_time = now 
        self.prev_prompt = False
        # Delay scheduling prompts to let waiting queue fill up
        if self.delay_factor > 0:
            earliest_arrival_time = min(
                [e.arrival_time for e in self.waiting_request_queue])
            # print("earliest_arrival_time")
            # print(earliest_arrival_time)
            # print("now")
            # print(now)
            # print("self.last_prompt_latency")
            # print(self.last_prompt_latency)
            # tt = now - earliest_arrival_time
            # print("tt")
            # print(tt)
            passed_delay = (
                (now - earliest_arrival_time) >
                (self.delay_factor * self.last_prompt_latency)) or (
                (now - earliest_arrival_time) ==
    ...
    def _continuous_batch_pa(self):
        ...
        ### 这里只是用padding先凑成batch,一开始里面的请求都是 invalid
    ...
    #     ServingBlockMemPool.instance().reset_budget()
    #     # ServingBlockMemPool.instance().log_status()
    #     ### 一开始推理时，先用 end_token填充batchsize个请求
    #     self.try_initialize_paddings_pa()
    #     # self.log_running_list("schedule start running status")
    #     # 判断batch内的running entry，能否进行本轮推理？
    #     num_entry_swapped_out = 0
    #     while not self.can_predict_current_batch(): ###看是否有足够的显存空间，如果空间不够就要减少请求
    #         # 如果不能，swap出去已有请求
    #         self.reset_all_budgets() ### 释放所有的budget
    #         self.try_swap_valid_entries() ### 使用padding替换一条正常的请求（目前是替换长度最短的那条）
    #         num_entry_swapped_out += 1
    #     if num_entry_swapped_out:
    #         self.reset_all_budgets()
    #         return
    ...
    #         # 如果有空batch槽，尝试插入
    #         # logging.debug("has new entry, trying to enter current batch")
    #         ### 不断将 waiting序列中的第一个请求去替换 running序列中的无效请求
    #         ### 这样就将running序列中的请求由一开始的padding变成了有效请求
    #         if not self.try_substitute_entry():
    #             # 尝试失败，退出
    #             break
    #     self.reset_all_budgets()
