...
class BlockMemPool:
    ...
    def allocate_block(self, num_new_block: int):
        ...
        self.free_blocks = self.free_blocks[num_new_block:] ### 从逻辑上是顺序分配的
        ### self.free_blocks在经过几次分配加资源收回后，里面的block的索引顺序应该也是乱的
        ### 但是没关系，只要和物理块的对应关系没变就行
...
### 每个请求都有一个CacheEngine，维护一个单独的block_table
class CacheEngine:
    ...
    def prepare_cache(self, num_new_token):
        ...
        if remained_token < num_new_token:
            ...
            ### 该序列的已分配 block不够用，向总资源池申请更多的block
...
class ServingCacheEngine(CacheEngine):
    def __init__(self, block_size: int, pool: BlockMemPool=None):
        ...
        self.num_budget_used = 0 ## 当前请求已经分配的block数量
    
    ### 看当前请求的已分配显存是否够用，不过不够，就看总资源池有没有剩余的显存，有的话也行
    ## 否则返回false，说明无论是已分配显存还是总的资源池中都不够用了
