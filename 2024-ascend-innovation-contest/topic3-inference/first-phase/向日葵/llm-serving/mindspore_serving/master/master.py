...
class BatchStrOut:
    def __init__(self,req_batch_id:int = None, token_str:str = None):
        self.req_id = req_batch_id
...
class AsyncMaster(Master):
    def __init__(
        ...
        self.detokenizer_que = asyncio.Queue()#queue.Queue()
        self.req_batch_id = 0
        self.request_engine = request_engine
        # self.detokenizer_threads = [
        #     threading.Thread(
        #         target=self.handle_detokenization_loop
        #     ) for _ in range(4)
        # ]
        # for t in self.detokenizer_threads:
        #     t.start()
    ...
    async def step_async(self) -> List[ResponseOutput]:
        ...
        if valid_entry_len == 0:
            return None
        await self._run_workers_async(current_batch_size, entry_metadata_list=entries_metadata_list)
        return 
