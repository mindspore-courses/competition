...
class Schedule:
    ...
    def __init__(self, config: ServingConfig):
        self.waiting_request_queue: List[EntryMetaData] = []#存储等待处理的推理请求队列。
    ...
    def add_entrys(self, entry_meta_data: EntryMetaData):
        ...
        logging.debug(f"add_entrys request get_max_token_len{entry_meta_data.entry_data.max_token_len}")
        # self.waiting_request_queue.append(entry_meta_data) tihuanheapq
    ...
    def _padding_request_into_batching_list(self, index):
        if not self.waiting_request_queue:
            ...
            if not self.waiting_request_queue:
                ...
                # data = self.waiting_request_queue.popleft()tihuanheapq
    ...
    def try_swap_valid_entries(self):
        ...
        # self.waiting_request_queue.appendleft(entry_to_swap) tihuanheapq
    ...
    def _continuous_batch_pa(self):
        logging.debug("_continuous_batch_pa")
        # 重置整个缓存池的预算。
        ...
        # 如果当前批次无法执行推理（例如，缓存不足），
        # 则调用 reset_all_budgets() 重置所有缓存资源，
        # 并调用 try_swap_valid_entries() 替换掉一个有效请求，确保资源足够进行推理。
    ...
    def upate_entries_after_one_step(self, outputs: List[int], eos_id: int, index_list: List[int] = None):
        ...
        # index_list 是可选的，如果提供了，表示这次更新只针对指定索引的请求。
