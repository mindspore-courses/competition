...
class WorkAgent:
    ...
    def predict_for_kbk(self, current_index, input_ids, valid_length, block_tables, slot_mapping):
        # 获取配置中的batchsize，假设从配置中获取
        batchsize = self.mindspore_model.config.prefill_batch_size[0]  # 从配置中获取 batchsize

        # 判断是否处于prefill阶段，仅在prefill时进行padding
        if self.is_prefill:
            input_batch_size = input_ids.shape[0]

            # 如果输入数据小于batchsize，则进行padding
            if input_batch_size < batchsize:
                # 对input_ids进行padding
                padding_size = batchsize - input_batch_size
                input_ids = np.vstack([input_ids, np.zeros((padding_size, input_ids.shape[1]), dtype=np.int64)])

                # 对valid_length进行padding
                valid_length = np.concatenate([valid_length, np.zeros(padding_size, dtype=np.int64)])

                # 对current_index进行padding
                current_index = np.concatenate([current_index, np.zeros(padding_size, dtype=np.int64)])

                # 对block_tables进行padding
                block_tables_padding = np.vstack(
                    [block_tables, np.zeros((padding_size, block_tables.shape[1]), dtype=np.int32)])
                block_tables = block_tables_padding

                # 对slot_mapping进行padding
                slot_mapping_padding = np.concatenate(
                    [slot_mapping, np.full(padding_size * slot_mapping.shape[1], -1, dtype=np.int32)])
                slot_mapping = slot_mapping_padding.reshape(-1, slot_mapping.shape[1])
