...
class BatchTokenIdOut:
    def __init__(self,output:str = None,entry_metadata_list = None,index_list = None, skip_inference = False):
...
class Master:
    ...
    def _llama_detokenizer_function(self, index, entry_metadata_list, skip_special_tokens=True):
        ...
        ########### add  liuyang
        # if prefix_index==0 and new_text.endswith("\n"):
        #     new_text = "\n"
        # all_text = self.tokenizer.decode(all_outputs_ids, skip_special_tokens=False)
        # txt_file = open('/home/ma-user/work/llm-serving/all_text.txt', 'a', encoding="utf-8")
        # if prefix_index==0:
        #     txt_file.write(all_text)
        #     txt_file.write("\n")
        ### read_index，每次保留一个阅读token
        #### 这里加打印不行，看看采用np.save的形式
        if len(new_text) > len(prefix_text) and not new_text.endswith("�"):
            ...
            ###############
            # logging.info("new_text tttttttttttttttttttttttt {}".format(new_text))
            # if prefix_index==0 and new_text.endswith("\n"):
            #     return "\n"
            # txt_file = open('/home/ma-user/work/llm-serving/new_text.txt', 'a', encoding="utf-8")
            # if prefix_index==0 and new_text.endswith("\n") and len(new_text)>2:
            #     txt_file.write(new_text[0:-2])
            #     txt_file.write("\n")
            return new_text
        else:
            return ""
    ...
    def _postprocess(self,
        ...
        #### 首token是无效token,更新状态后返回，不要解码 liuyang
        ...
        if index_list is not None:
            ...
            for idx, index in enumerate(index_list):
                ### liuyang
                # if entry_metadata_list[index].get_entry_data().get_output_len()==1:
                #     continue
    ...
    def add_requests_to_schedule_pool(self,
        ...
        ### add liuyang
        # txt_file = open('/home/ma-user/work/llm-serving/new_request.txt', 'a', encoding="utf-8")
        # txt_file.write(request_id)
        # txt_file.write("\n")
    ...
    def _mock_run_workers_async(self, batch_size: int):
        outputs = []
        for i in range(batch_size):
            output = random.randint(0, 32000) ### 生成了一个随机token
            # output = 100 ##### liuyang
...
class AsyncMaster(Master):
    ...
    def _get_prompt_batch_list(self, entry_metadata_list):
        ...
        ### 这里 index_list是指 整个running_request_list中为 prompt请求的下标
        ...
        if prompt_data_count == 0: ### 说明当前的decode_batchsize个请求中没有 prompt请求，返回空列表
