...
class WorkAgent:
    ...
    def predict(self, shape_list=None, current_batch=None, batch_valid_flag=None):
        ...
        if self.is_prefill:
            ...
            # 获取first_group数组中的有效长度valid_length_，并根据配置将其转换为int64或int32类型的数组valid_length。
            ...
            # 获取当前decode_params_map的大小，即之前的批次大小。
            ...
            else:#如果之前的批次大小大于当前批次大小，需要收缩decode_params_map
                # pop
            ...
            for key in self.decode_params_map.keys():
                ...
                # 将初始化重置标志和解码索引分别添加到init_reset和decode_index列表中。
                init_reset.append(decode_params.init_reset)
                decode_index.append(decode_params.decode_index)
            ...
            # 加入PA特性，获取block_tables_shape和slot_mapping_shape。
            if self.config.model_config.page_attention:
        ...
        if self.config.model_config.backend == "ge":
            ...
            # decode 时，先将 shape 与 prefill 改为一致。如果input_ids的形状为1列，扩展其形状以与预填充状态一致。
...
# 用于启动agent的socket服务器，接受客户端连接并处理推理请求
def start_agent_socket_server(i, cfg: ServingConfig, startup_queue):
    ...
    server.listen(5)#开始监听连接，允许最多5个待处理连接
    ...
    while True:
        if not parent_process.is_running():
            ...
            server.close()
            return
        # 接受客户端连接，并获取连接对象和客户端地址。
        conn, client_addr = server.accept()
        # todo workagent = WorkAgent(config)
        while True:
            # 如果父进程不再运行，记录一条警告日志，并关闭服务器，退出循环。
            if not parent_process.is_running():
                logging.warning(
            ...
            try:
                ...
                # worker 和 agent建联
                if data.startswith('#'):
                    # 检查work_agent的状态是否为未连接状态。如果是未连接状态，处理数据，更新共享内存名称列表，将状态设置为已连接，并发送成功消息。
                ...
                elif data.startswith('*'):#如果数据以*开头，表示全量推理请求。
                    # 全量推理，设置work_agent为预填充状态。
                    ...
                    # 调用work_agent.predict执行推理。
                    _, _ = work_agent.predict(shape_list=input_shapes)
                    # 如果是第一个agent，发送成功消息。
                    if i == 0:
                        conn.sendall("1".encode())
                elif data.startswith('a'):
                    # 增量推理，如果数据以a开头，表示增量推理请求。解析数据，获取当前批次大小和有效标志。
                    decode_data = data.split('_')
                    # 增加PA的判断，如果配置了Page Attention（PA），获取块表和插槽映射的形状。
...
# 用于启动多个agent进程。它会为每个agent端口创建一个新进程，并启动agent的socket服务器。
