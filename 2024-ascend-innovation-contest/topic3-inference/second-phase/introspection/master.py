...
class Master:
    def __init__(self,
        ...
        self.decode_cache = {}
        self.scheduler.get_prefill_ratio(self.prefill_ratio)
...
class AsyncMaster(Master):
    ...
    def _get_prompt_batch_list(self, entry_metadata_list,batch_size):
        # PA取一个data进行prefill
        if self.config.model_config.page_attention:
            return self._check_prompt_predict_data_pa(entry_metadata_list,batch_size)
    ...
    ##changed
    @staticmethod
    def _check_prompt_predict_data_pa(entry_metadata_list,batch_size):
        ...
        for index, item in enumerate(entry_metadata_list):
            ...
            if len(index_list)>=batch_size//64:
                break

        logging.debug(f"GFtest: batch_size {batch_size}, entry_metadata_list length {len(entry_metadata_list)}, input_entry_metadata_list {len(input_entry_metadata_list)}")
        return input_entry_metadata_list, index_list
    ...
    async def _run_workers_async(self, current_batch_size, entry_metadata_list):
        ...
        input_entry_metadata_list, index_list = self._get_prompt_batch_list(entry_metadata_list,current_batch_size)
        logging.debug("_get_prompt_batch_list prompt index_list {}, input_entry_metadata_list {}"
                      .format(index_list, input_entry_metadata_list))
        # prefill predict
