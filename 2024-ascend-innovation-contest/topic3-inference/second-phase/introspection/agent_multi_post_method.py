...
from tools.post_sampling_model import temperature_TopK

pool = ThreadPoolExecutor(max_workers=20, thread_name_prefix='test_thread')
default_temperature = np.array([1])

# logging.basicConfig(filename='agent_muti.log',format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',level=logging.INFO)
...
def send_config(config):
    prefill_ratio=config.model_config.decode_batch_size[0]//64
    n=2
    if prefill_ratio==0:
        prefill_ratio=1
        n=1
    seq_length=config.model_config.seq_length[0]
    config.model_config.seq_length = [seq_length//(prefill_ratio*n)]
    config.pa_config.decode_seq_length = seq_length//(prefill_ratio*n)
...
class WorkAgent:
    ...
    def predict_for_kbk(self, current_index, input_ids, valid_length, block_tables, slot_mapping):
        ...
        if self.mindspore_model.config.use_past:
            logging.debug(f"predict before pa predict_for_kbk prefill {input_ids} {valid_length}. first phase {self.mindspore_model.phase}")
            if self.is_prefill:
                ...
                res = self.mindspore_model(input_ids=Tensor(input_ids, mstype.int32),input_position=Tensor(current_index2,mstype.int32),init_reset=Tensor([False], mstype.bool_),batch_valid_length=Tensor([valid_length], mstype.int32),block_tables= Tensor(block_tables, mstype.int32),slot_mapping=Tensor(slot_mapping, mstype.int32))
        ...
        # logging.info(f"mindspore_model_input:{model_input},org_input:{org_input}")
