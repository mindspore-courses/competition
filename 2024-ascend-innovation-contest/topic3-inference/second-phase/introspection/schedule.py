...
class Schedule:
    ...
    def __init__(self, config: ServingConfig):
        ...
        self.max_invaild_num=self.batch_size//64
        self.invalid_flag=True
        
    def get_prefill_ratio(self,prefill_ratio):
        self.max_invaild_num = self.batch_size//64
    ...
    def _insert_new_prompt_to_batch_pa(self, index):
        ...
        data.get_entry_data().set_decode_index(index)
        self.running_request_list[index] = data
        logging.debug(f'add new valid request in batch, batch size index is {index}, is prefill? {data.is_prompt} {data.entry_data.status == EntryStatus.INPUT_OUTOFRANGE}')
    ##changed
    def try_substitute_entry(self):
        ...
        if not is_invalid_index_list:
            ...
            return False
        # 如果有空槽位，尝试替代一条新请求：
        # 如果新槽位的数量大于self.max_invalid_num,再尝试发送新请求
        if len(is_invalid_index_list)>self.max_invaild_num:
            self.invalid_flag=True

        if self.invalid_flag:
            logging.debug(f'GFtest: is_invalid_index_list length {len(is_invalid_index_list)}')
            ...
            if new_entry.cache_engine.try_use_budget(new_entry.get_entry_data().get_len()):
                ...
                if len(is_invalid_index_list) == 1:
                    logging.debug(f'GFtest: set invalid_flag False')
