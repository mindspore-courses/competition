...
from mindspore import ops as orgin_ops
from mindspore import dtype as mstype
#import mindspore.mint.nn.functional as F
from mindspore.ops import Equal
...
class MixtralRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000):
        ...
        self.base = base
        inv_freq = 1.0 / (self.base ** (mint.arange(0, self.dim, 2, dtype=mindspore.int32).float() / self.dim))
    ...
    def _set_cos_sin_cache(self, seq_len, dtype):
        ...
        t = mint.arange(self.max_seq_len_cached, dtype=mindspore.int32).type_as(self.inv_freq)
...
class MixtralAttention(nn.Module):
    ...
    def _shape(self, tensor: mindspore.Tensor, seq_len: int, bsz: int):
        x = ops.reshape(tensor, (bsz, seq_len, self.num_heads, self.head_dim))
        return ops.transpose(x, 1, 2).contiguous()
    ...
    def forward(
        ...
        if attention_mask is not None:  # no matter the length, we just slice it
            ...
            split_size = key_states.shape[-2]  # 获取 key_states 的倒数第二个维度大小
            splits = mint.split(attention_mask, [split_size, attention_mask.shape[-1] - split_size], -1)  # 拆分最后一个维度
            causal_mask = splits[0]  # 选择拆分后的第一个部分
...
class MixtralFlashAttention2(MixtralAttention):
    ...
    def forward(
        ...
        #attn_mask = Tensor(np.ones((bsz, self.num_heads, q_len, kv_seq_len)), dtype=mindspore.float16).bool()


        attn_output = self._flash_attention_forward(
            query_states, key_states, value_states, attn_mask
        )
        attn_output = ops.transpose(attn_output, 1, 2).contiguous()
    ...
    def _flash_attention_forward(
        self, query_states, key_states, value_states, attn_mask):
        ...
        #ones = mint.ones((2048, 2048), dtype=mindspore.int32)
        #attn_mask = ops.triu(ones, diagonal=1).bool()

        #print("attn_mask.shape:",attn_mask.shape)

        model = FlashAttentionScore(head_num,
                                keep_prob=1.0,
                                pre_tokens=2147483647,
                                next_tokens=2147483647,
                                input_layout="BNSD",
                                sparse_mode=3,
                                scale_value=1.0 / math.sqrt(query_states.shape[-1]),
                                )
        attn_output =model(
                    query_states,
                    key_states,
        ...
        return attn_output.to(query_states.dtype)
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def process_expert(expert_layer, current_state, routing_weight):
        return expert_layer(current_state) * routing_weight

    def forward(self, hidden_states: mindspore.Tensor) -> mindspore.Tensor:
        batch_size, sequence_length, hidden_dim = hidden_states.shape

        if self.training and self.jitter_noise > 0:
            hidden_states *= ops.uniform(hidden_states.shape, 1.0 - self.jitter_noise, 1.0 + self.jitter_noise)
        ...
        expert_mask = mint.nn.functional.one_hot(selected_experts, num_classes=self.num_experts)
        expert_mask = mint.permute(expert_mask,(2, 1, 0))
        # 计算所有 expert_mask 的非零索引，并提前转换为所需的 dtype
        ...
        **kwargs,
        ...
        if past_key_values is not None:
            if inputs_embeds is not None:  # Exception 1
                if 0 not in mindspore.ops.shape(input_ids):
                        input_size = mindspore.ops.shape(input_ids)[1]
                        cache_size = mindspore.ops.shape(cache_position)[0]
                        indices = mint.arange(input_size - cache_size, input_size, dtype=mindspore.int32)
                        # 在 input_ids 的第 1 维度上选择索引范围内的数据
                        input_ids = mint.index_select(input_ids, 1, indices)                
            elif mindspore.ops.shape(input_ids)[1] != mindspore.ops.shape(cache_position)[0]:  # Default case (the "else", a no op, is Exception 2)
                # 在 input_ids 上使用 cache_position 进行索引
        ...
        if attention_mask is not None and position_ids is None:
            ...
            position_ids = mint.cumsum(attention_mask.astype(mindspore.int32), -1) - 1

            # position_ids = position_ids.masked_fill(attention_mask == 0, 1)
            mask_condition = mint.eq(attention_mask, 0)
            position_ids = ops.masked_fill(position_ids, mask_condition, 1)
            if past_key_values:
                #position_ids = position_ids[:, -input_ids.shape[1] :]
                pos_size = mindspore.ops.shape(position_ids)[1]  # 获取 position_ids 第二维的大小
                indices = mint.arange(pos_size - input_ids.shape[1], pos_size)  # 构造范围索引
                position_ids = mint.index_select(position_ids, 1, indices)  # 通过 index_select 选择最后 input_ids.shape[1] 列
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                ...
                sequence_lengths = mint.eq(input_ids, self.config.pad_token_id)
                sequence_lengths = mint.argmax(sequence_lengths.astype(mindspore.int32),-1) -1 
