...
class Qwen2MoeRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        if hidden_states.dtype==self.weight.dtype:
            ...
            return F.rms_norm(hidden_states.to(self.weight.dtype), self.weight, self.variance_epsilon).to(dtype)
...
class Qwen2MoeSparseMoeBlock(nn.Module):
    ...
    def stack_moe(self):
        self.w1_weight = ops.swapaxes(mindspore.Tensor(mint.stack([expert.gate_proj.weight for expert in self.experts])), 2, 1)
        self.w2_weight = ops.swapaxes(mindspore.Tensor(mint.stack([expert.down_proj.weight for expert in self.experts])), 2, 1)
        self.w3_weight = ops.swapaxes(mindspore.Tensor(mint.stack([expert.up_proj.weight for expert in self.experts])), 2, 1)
        # del self.experts
        self.stacked = True
    ...
    def moe_fast_forward(self, hidden_states, expert_mask, routing_weights):
        ...
        hidden_states = mint.matmul(hidden_states, self.w3_weight)
        hidden_states = activated_output * hidden_states
        hidden_states = mint.matmul(hidden_states, self.w2_weight)
        ...
        hidden_states = mint.sum((mask1+mask2) * hidden_states,dim=0)
