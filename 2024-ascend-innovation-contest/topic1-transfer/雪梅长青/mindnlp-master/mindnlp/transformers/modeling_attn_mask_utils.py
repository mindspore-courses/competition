...
class AttentionMaskConverter:
    ...
    def to_causal_4d(
        ...
        # past_key_values_length = mindspore.mint.sub(key_value_length, query_length)
    ...
    def to_4d(
        ...
        if causal_4d_mask is not None:
            expanded_attn_mask = causal_4d_mask.masked_fill(expanded_attn_mask.bool(),
                                                            float(np.finfo(mindspore.dtype_to_nptype(dtype)).min))
    ...
    def _make_causal_mask(
        ...
        mask = mask.masked_fill(mindspore.mint.less(mask_cond, (mask_cond + 1).view(mask.shape[-1], 1)), 0).to(dtype)

        if past_key_values_length > 0:
            mask = mindspore.mint.cat([mindspore.mint.zeros(tgt_len, past_key_values_length, dtype=dtype), mask],
                                      dim=-1)
        # add lower triangular sliding window mask if necessary
        if sliding_window is not None:
            diagonal = past_key_values_length - sliding_window + 1

            context_mask = 1 - ops.triu(mindspore.mint.ones_like(mask, dtype=mindspore.int32), diagonal=diagonal)
            mask = mask.masked_fill(context_mask.bool(),
                                    mindspore.tensor(np.finfo(mindspore.dtype_to_nptype(dtype)).min))

        return mindspore.mint.broadcast_to(mask[None, None, :, :], (bsz, 1, tgt_len, tgt_len + past_key_values_length))
    ...
    def _expand_mask(mask: mindspore.Tensor, dtype, tgt_len: Optional[int] = None):
        ...
        # expanded_mask = mask[:, None, None, :].broadcast_to((bsz, 1, tgt_len, src_len))
        expanded_mask = mindspore.mint.broadcast_to(mask[:, None, None, :], (bsz, 1, tgt_len, src_len)).to(dtype)
