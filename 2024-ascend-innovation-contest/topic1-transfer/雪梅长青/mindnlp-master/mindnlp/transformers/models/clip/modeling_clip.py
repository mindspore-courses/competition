...
def contrastive_loss(logits: mindspore.Tensor) -> mindspore.Tensor:
    return nn.functional.cross_entropy(logits, mindspore.mint.arange(len(logits)))
...
class CLIPVisionEmbeddings(nn.Module):
    def __init__(self, config: CLIPVisionConfig):
        ...
        self.class_embedding = nn.Parameter(np.random.randn(self.embed_dim))
        ...
        self.position_embedding = Embedding(self.num_positions, self.embed_dim)
    ...
    def forward(self, pixel_values: mindspore.Tensor) -> mindspore.Tensor:
        ...
        patch_embeds = mindspore.mint.permute(ops.flatten(patch_embeds, 2), (0, 2, 1))  # b, hw, c

        class_embeds = mindspore.mint.broadcast_to(self.class_embedding, (batch_size, 1, -1))
        embeddings = mindspore.mint.cat([class_embeds.to(patch_embeds.dtype), patch_embeds], dim=1)
        embeddings = mindspore.mint.add(embeddings, self.position_embedding(self.position_ids))
        return embeddings  # b, hw+1, c
...
class Embedding(nn.Module):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, padding_idx: Optional[int] = None,
        ...
        if _weight is None:
            self.weight = nn.Parameter(ops.empty((num_embeddings, embedding_dim), **factory_kwargs),
    ...
    def forward(self, input: Tensor) -> Tensor:
        return mindspore.ops.auto_generate.gen_ops_prim.embedding_op(
            input, self.weight, self.padding_idx, self.max_norm, self.norm_type, self.scale_grad_by_freq)
...
class CLIPTextEmbeddings(nn.Module):
    def __init__(self, config: CLIPTextConfig):
        ...
        self.token_embedding = Embedding(config.vocab_size, embed_dim)
        self.position_embedding = Embedding(config.max_position_embeddings, embed_dim)
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            position_ids = mindspore.mint.narrow(self.position_ids, 1, 0, seq_length)

        if inputs_embeds is None:
            inputs_embeds = self.token_embedding(input_ids)

        position_embeddings = self.position_embedding(position_ids)
        embeddings = mindspore.mint.add(inputs_embeds, position_embeddings)
...
class CLIPAttention(nn.Module):
    ...
    def _shape(self, tensor: mindspore.Tensor, seq_len: int, bsz: int):
        return mindspore.mint.permute(tensor.view(bsz, seq_len, self.num_heads, self.head_dim), (0, 2, 1, 3))
    ...
    def forward(
        ...
        # get query proj
        query_states = mindspore.mint.mul(self.q_proj(hidden_states), self.scale)
        ...
        attn_weights = mindspore.mint.bmm(query_states, mindspore.mint.permute(key_states, (0, 2, 1)))
        ...
        attn_output = mindspore.mint.permute(attn_output, (0, 2, 1, 3))
...
class FlashAttention(nn.Module):
    def __init__(self, num_heads, head_dim, dropout_rate=0.0, prev_tockens=65536, next_tockens=65536):
        ...
        if self.keep_prob < 1.0:
            ...
            self.drop_gen_mask = ops.ops.DropoutGenMask()

    def forward(self, query, key, value, attention_mask):
        bsz, head_num, seq_len, _ = query.shape
        if self.keep_prob < 1.0:
            seq_len_g = (seq_len // 8) * 8
            if seq_len_g < 8:
                drop_mask = ops.reshape(self.drop_gen_mask((bsz, head_num, seq_len, 8), self.keep_prob_tensor),
                                        (bsz, head_num, seq_len, 0))
            else:
                drop_mask = ops.reshape(self.drop_gen_mask((bsz, head_num, seq_len, seq_len_g), self.keep_prob_tensor),
                                        (bsz, head_num, seq_len, seq_len_g // 8))
        else:
            drop_mask = None
        if attention_mask is not None:
            attention_mask = ops.reshape(attention_mask, (bsz, -1, seq_len, seq_len)).to(mstype.uint8)
        _, _, _, attention = self.flash_attention(query, key, value, None, drop_mask, None, attention_mask, None)
        return attention


class CLIPFlashAttention2(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def __init__(self, config):
        ...
        self.q_proj = Linear(self.embed_dim, self.embed_dim)
        self.out_proj = Linear(self.embed_dim, self.embed_dim)
        self.flash_attention = FlashAttention(num_heads=self.num_heads, head_dim=self.head_dim,
    ...
    def forward(
        ...
        key_states = self._shape(self.k_proj(hidden_states), -1, bsz)
        value_states = self._shape(self.v_proj(hidden_states), -1, bsz)
        query_states = self._shape(query_states, tgt_len, bsz)  # b, n, s, d

        target_type = self.q_proj.weight.dtype
        if query_states.dtype != mstype.float16:
            query_states = query_states.to(mstype.float16)
            key_states = key_states.to(mstype.float16)
            value_states = value_states.to(mstype.float16)

        if causal_attention_mask is not None:
            if attention_mask is not None:
                attention_mask = mindspore.mint.add(attention_mask, causal_attention_mask)
            else:
                attention_mask = causal_attention_mask
        attn_output = self.flash_attention(query_states, key_states, value_states, attention_mask)
        if attn_output.dtype != target_type:
            attn_output = attn_output.to(target_type)
...
class QuickGELUActivation(nn.Module):
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        Raises:
            None
        """
        # return input * mindspore.mint.sigmoid(1.702 * input)
        return mindspore.mint.mul(input, mindspore.mint.sigmoid(mindspore.mint.mul(1.702, input)))
...
class Linear(nn.Module):
    def __init__(self, in_features, out_features, bias=True, dtype=None) -> None:
        ...
        self.weight = nn.Parameter(ops.empty((out_features, in_features), **factory_kwargs))
        if bias:
            self.bias = nn.Parameter(ops.empty(out_features, **factory_kwargs))
        else:
            self.register_parameter('bias', None)

    def forward(self, input):
        return mindspore.mint.nn.functional.linear(input, self.weight, self.bias)
...
class LayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-5, elementwise_affine=True, bias: bool = True,dtype=None):
        ...
        if self.elementwise_affine:
            ...
            if bias:
                self.bias = nn.Parameter(ops.empty(self.normalized_shape, **factory_kwargs))
            else:
                self.register_parameter('bias', None)
        else:
...
class CLIPEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = mindspore.mint.add(residual, hidden_states)
    ...
    def forward(
        ...
        if self.eos_token_id == 2:
            ...
            pooled_output = last_hidden_state[
                ...
                mindspore.mint.argmax(input_ids, dim=-1),
            ]
        else:
            ...
            pooled_output = last_hidden_state[
                ...
                mindspore.mint.argmax((mindspore.mint.eq(input_ids, self.eos_token_id)).int(), -1),
            ]
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.visual_projection = Linear(self.vision_embed_dim, self.projection_dim, bias=False)
        self.text_projection = Linear(self.text_embed_dim, self.projection_dim, bias=False)
    ...
    def forward(
        ...
        logit_scale = mindspore.mint.exp(self.logit_scale)
        logits_per_text = mindspore.mint.mul(mindspore.mint.matmul(text_embeds, image_embeds.t()), logit_scale)
...
class CLIPVisionModelWithProjection(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPVisionConfig):
        ...
        self.visual_projection = Linear(config.hidden_size, config.projection_dim, bias=False)
...
class CLIPForImageClassification(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig) -> None:
        ...
        # Classifier head
        self.classifier = (
            Linear(config.vision_config.hidden_size, config.num_labels) if config.num_labels > 0 else nn.Identity()
        )
