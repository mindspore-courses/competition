...
class T5Attention(nn.Module):
    ...
    def compute_bias(self, query_length, key_length):
        ...
        # context_position = ops.arange(query_length, dtype=mindspore.int32)[:, None]
        # memory_position = ops.arange(key_length, dtype=mindspore.int32)[None, :]
        context_position = ops.arange(query_length, dtype=mindspore.int32).unsqueeze(1)
        memory_position = ops.arange(key_length, dtype=mindspore.int32).unsqueeze(0)
    ...
    def forward(
        ...
        if position_bias is None:
            ...
            if past_key_value is not None:
                ...
                start_idx = position_bias.shape[2] - hidden_states.shape[1]
                position_bias = ops.narrow(position_bias, 2, start_idx, hidden_states.shape[1])
        ...
        if self.pruned_heads:
            ...
            indices = mindspore.tensor(pruned_heads, dtype=mindspore.int32)
            ops.scatter(mask, 0, indices, 0)
            position_bias_masked = position_bias[:, mask.bool()]
            # position_bias_masked = ops.where(mask.bool(), position_bias, mindspore.tensor(0.0))
        else:
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _shift_right(self, input_ids):
        ...
        input_ids_narrowed = ops.narrow(input_ids, -1, 0, input_ids.shape[-1] - 1)
        shifted_input_ids = ops.cat([shifted_input_ids, input_ids_narrowed], dim=-1)
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            ...
            if encoder_attention_mask is None:
                encoder_attention_mask = ops.ones(
                    encoder_hidden_shape, dtype=mindspore.int32
                )
