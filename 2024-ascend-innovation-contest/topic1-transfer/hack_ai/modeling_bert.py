...
class BertSelfAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        ...
        self.attention_head_size_sqrt = mindspore.Tensor(math.sqrt(self.attention_head_size))
    ...
    def forward(
        ...
        # attention_scores = attention_scores / ops.sqrt(mindspore.Tensor(self.attention_head_size, dtype=attention_scores.dtype))
        attention_scores = attention_scores / self.attention_head_size_sqrt.astype(attention_scores.dtype)
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        if self.chunk_size_feed_forward > 0:
            num_chunks = attention_output[0].shape[self.seq_len_dim] // self.chunk_size_feed_forward

            # chunk input tensor into tuples
            input_tensors_chunks = tuple(ops.chunk(input_tensor, num_chunks, dim=self.seq_len_dim) for input_tensor in attention_output)
            # apply forward fn to every tuple
            forward_fn = self.feed_forward_chunk
            output_chunks = tuple(forward_fn(*input_tensors_chunk) for input_tensors_chunk in zip(*input_tensors_chunks))
            # concatenate output at same dimension
            layer_output = ops.cat(output_chunks, dim=self.seq_len_dim)
        else:
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer_list = [BertLayer(config) for _ in range(config.num_hidden_layers)]
        self.layer = nn.ModuleList(self.layer_list)
        self.layer_len = config.num_hidden_layers
    ...
    def forward(
        ...
        # layer_module = self.layer_list[0]
        ...
        for i in range(self.layer_len):
            layer_module = self.layer_list[i]
