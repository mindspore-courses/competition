...
class BertModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_time(self):
        import time
        mindspore.set_context(mode=0, enable_graph_kernel=True, pynative_synchronize=False)
        
        @mindspore.jit(jit_config=mindspore.JitConfig(jit_syntax_level='STRICT'))
        def infer(input_ids, attention_mask):
            return model(input_ids, attention_mask=attention_mask, return_dict=False)[0]
        ...
        print(infer_time)
        avg_time = np.array(infer_time)[1:].mean()
