...
class T5Attention(nn.Module):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.relative_attention_max_distance = mindspore.Tensor(config.relative_attention_max_distance, dtype=mindspore.float32)
        ...
        if self.has_relative_attention_bias:
            self.relative_attention_bias = nn.Embedding(self.relative_attention_num_buckets, self.n_heads).jit()
            
            
            # relative_position = []
            # for i in range(2048):
            #     relative_position.append(np.arange(2048)-i)
            # self.relative_position = np.array(relative_position)
            
            
            self.compute_bias_cache_dict = {}
            for i in range(1, 513): #空间换时间，如果显存够大，可以再加大
                position_bias = self.compute_bias_cache(i, i)
                self.compute_bias_cache_dict[i] = position_bias
    ...
    def _relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        ...
        relative_position_if_large = max_exact + (
            ...
            / ops.log(max_distance / max_exact)
    ...
    def compute_bias(self, query_length, key_length):
        ...
        # relative_position = self.relative_position[:query_length, :key_length]
        # relative_position = mindspore.Tensor(relative_position)
    ...
    def compute_bias_cache(self, query_length, key_length):
        context_position = ops.arange(query_length, dtype=mindspore.int64)[:, None]
        memory_position = ops.arange(key_length, dtype=mindspore.int64)[None, :]
        relative_position = memory_position - context_position  # shape (query_length, key_length)
    ...
    def forward(
        ...
        # print(f'real_seq_length:{real_seq_length}')
        ...
        if position_bias is None:
            if not self.has_relative_attention_bias:
                ...
                if (real_seq_length==key_length) and (real_seq_length in self.compute_bias_cache_dict.keys()):
                    relative_position_bucket = self.compute_bias_cache_dict[real_seq_length]
                    values = self.relative_attention_bias(relative_position_bucket)  # shape (query_length, key_length, num_heads)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config):
        ...
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon).jit()
        self.dropout = nn.Dropout(config.dropout_rate)
    
    # @mindspore.jit(jit_config=mindspore.JitConfig(jit_syntax_level='STRICT'))
    def step_1(self,hidden_states ):
        normed_hidden_states = self.layer_norm(hidden_states)
    ...
    def forward(
        ...
        normed_hidden_states = self.step_1(hidden_states)
        # print(f'normed_hidden_states:{normed_hidden_states.shape}')
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.is_decoder = config.is_decoder
        self.layer_list = []
        self.layer_list.append(T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias))
        if self.is_decoder:
            self.layer_list.append(T5LayerCrossAttention(config))
        
        self.layer_list.append(T5LayerFF(config))
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            # print(f'self_attn_past_key_value:{mindspore.Tensor(list(self_attn_past_key_value)).shape}')
            # print(f'cross_attn_past_key_value:{mindspore.Tensor(list(cross_attn_past_key_value)).shape}')
        else:
            self_attn_past_key_value, cross_attn_past_key_value = None, None

        self_attention_outputs = self.layer_list[0](
            hidden_states,
        ...
        # print(f'do_cross_attention:{do_cross_attention}')
        if do_cross_attention:
            ...
            cross_attention_outputs = self.layer_list[1](
                hidden_states,
        ...
        hidden_states = self.layer_list[-1](hidden_states)
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.embed_tokens = embed_tokens.jit()
        self.is_decoder = config.is_decoder
        
        self.block_list = [T5Block(config, has_relative_attention_bias=bool(i == 0)) for i in range(config.num_layers)]
        self.block = nn.ModuleList(self.block_list)
        self.block_len = config.num_layers
    ...
    # @mindspore.jit(jit_config=mindspore.JitConfig(jit_syntax_level='STRICT'))
    def step_1(self, input_ids, use_cache, output_attentions, output_hidden_states, return_dict, inputs_embeds):
        # Model parallel
        ...
        if inputs_embeds is None:
            ...
            # inputs_embeds = mindspore.ops.gather(self.embed_tokens.weight, input_ids, 0)

        batch_size, seq_length = input_shape
        
        # print(f'inputs_embeds:{inputs_embeds.shape}')
        
        return batch_size, seq_length, input_shape, inputs_embeds, return_dict, output_hidden_states, output_attentions, use_cache
    ...
    def forward(
        ...
        batch_size, seq_length, input_shape, inputs_embeds, return_dict, output_hidden_states, output_attentions, use_cache = self.step_1(input_ids, use_cache, output_attentions, output_hidden_states, return_dict, inputs_embeds)
        ...
        if past_key_values is None:
            past_key_values = [None] * self.block_len
        ...
        for i in range(self.block_len):
            layer_module = self.block_list[i]
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # if past_key_value is not None:
                #     print(f'{len(list(past_key_value))}')
                #     print(f'past_key_value:{mindspore.Tensor(past_key_value[0]).shape}')
                #     print(f'past_key_value:{mindspore.Tensor(past_key_value[1]).shape}')
                #     print(f'past_key_value:{mindspore.Tensor(past_key_value[2]).shape}')
                #     print(f'past_key_value:{mindspore.Tensor(past_key_value[3]).shape}')
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5Stack(decoder_config, self.shared).jit()

        self.lm_head = nn.Linear(config.d_model, config.vocab_size, bias=False).jit()
        ...
        self.gen_tokens = 1
        self.max_decoder_input_ids = None
        self.use_static_infer = False
        self.max_static_infer_tokens = 200
    ...
    def forward(
        ...
        if self.use_static_infer and self.gen_tokens<=self.max_static_infer_tokens:
            step_len = 50
            
            if self.max_decoder_input_ids is None:
                self.max_decoder_input_ids = []
                for i in range(0,10000):
                    max_decoder_input_ids = ops.zeros((decoder_input_ids.shape[0], (i+1)*step_len)).astype(decoder_input_ids.dtype)
                    max_decoder_input_ids[:, :decoder_input_ids.shape[1]] = decoder_input_ids
                    max_lm_logits = self.forward_static_infer(encoder_outputs.last_hidden_state,attention_mask,max_decoder_input_ids)
                    self.max_decoder_input_ids.append(max_decoder_input_ids)
                    # print(f'=================={i}==================')
                    if (i+1)*step_len>=self.max_static_infer_tokens:
                        break
            else:
                max_decoder_input_ids = self.max_decoder_input_ids[int((self.gen_tokens-1)/step_len)]
                max_decoder_input_ids[:, :decoder_input_ids.shape[1]] = decoder_input_ids
                max_lm_logits = self.forward_static_infer(encoder_outputs.last_hidden_state,attention_mask,max_decoder_input_ids)
            
            lm_logits = max_lm_logits[:, :self.gen_tokens, :]
