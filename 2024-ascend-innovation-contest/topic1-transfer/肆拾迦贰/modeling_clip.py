...
def contrastive_loss(logits: mindspore.Tensor) -> mindspore.Tensor:
    return nn.functional.cross_entropy(logits, mint.arange(len(logits)))
...
class CLIPTextEmbeddings(nn.Module):
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            # position_ids = ops.narrow(self.position_ids, 1, 0, seq_length)
...
class CLIPAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights = mint.bmm(query_states, ops.transpose(key_states, 1, 2))
...
class CLIPEncoder(nn.Module):
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            layer_outputs_tuple  +=  (layer_outputs, )
...
class CLIPTextTransformer(nn.Module):
    def __init__(self, config: CLIPTextConfig):
        ...
        self.encoder = CLIPEncoder(config).jit()
    ...
    def forward(
        ...
        for layer_outputs in layer_outputs_tuple:
            if output_attentions:
                all_attentions = all_attentions + (layer_outputs[1],)
            if output_hidden_states:
                encoder_states = encoder_states + (layer_outputs[0],)
        ...
        if self.eos_token_id == 2:
            ...
            pooled_output = last_hidden_state[
                ...
                mint.argmax((input_ids.to(dtype=mindspore.int32) == self.eos_token_id).int(), -1),
            ]
...
# no use
# @mindspore.jit(jit_config=mindspore.JitConfig(jit_syntax_level='STRICT'))
def CLIPTextTransformer_jit(text_model,
            input_ids,
            attention_mask,
            position_ids,
            output_attentions,
...
class CLIPTextModel(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        # return CLIPTextTransformer_jit(self.text_model,
        #     input_ids=input_ids,
        #     attention_mask=attention_mask,
        #     position_ids=position_ids,
        #     output_attentions=output_attentions,
...
# @mindspore.jit(jit_config=mindspore.JitConfig(jit_syntax_level='STRICT'))
def model_vision_jit(vision_model, pixel_values, output_attentions, output_hidden_states, return_dict):
    return vision_model(
            pixel_values=pixel_values,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
...
# @mindspore.jit(jit_config=mindspore.JitConfig(jit_syntax_level='STRICT'))
def model_text_jit(text_model, input_ids, attention_mask, position_ids, output_attentions,
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        # vision_outputs = model_vision_jit(self.vision_model,
        #     pixel_values=pixel_values,
        #     output_attentions=output_attentions,
        #     output_hidden_states=output_hidden_states,
        #     return_dict=return_dict,
        ...
        # text_outputs = model_text_jit(self.text_model,
        #     input_ids=input_ids,
        #     attention_mask=attention_mask,
        #     position_ids=position_ids,
        #     output_attentions=output_attentions,
...
class CLIPForImageClassification(CLIPPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = mint.mean(sequence_output[:, 1:, :], dim=1)
        # sequence_output = ops.mean(ops.narrow(sequence_output, 1, 1, sequence_output.shape[1]), dim=1)
        # apply classifier
