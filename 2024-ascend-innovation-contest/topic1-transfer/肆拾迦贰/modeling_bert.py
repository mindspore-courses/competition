...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "token_type_ids", mint.zeros(self.position_ids.shape, dtype=mindspore.int64), persistent=False
        )
    ...
    def forward(
        ...
        if position_ids is None:
            ...
            position_ids = mint.narrow(self.position_ids, 1, past_key_values_length, seq_length)
        ...
        if token_type_ids is None:
            if hasattr(self, "token_type_ids"):
                ...
                buffered_token_type_ids = mint.narrow(buffered_token_type_ids, 1, 0, seq_length)
                buffered_token_type_ids_expanded = mint.broadcast_to(buffered_token_type_ids, (input_shape[0], seq_length))

                token_type_ids = buffered_token_type_ids_expanded
            else:
                token_type_ids = mint.zeros(input_shape, dtype=mindspore.int64)
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = mint.cat([past_key_value[0], key_layer.to(past_key_value[0].dtype)], dim=2)
            value_layer = mint.cat([past_key_value[1], value_layer.to(past_key_value[1].dtype)], dim=2)
        else:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))
        ...
        attention_scores = mint.matmul(query_layer, ops.transpose(key_layer, -1, -2))
        ...
        if self.position_embedding_type in ('relative_key', 'relative_key_query'):
            ...
            if self.position_embedding_type == "relative_key":
                ...
                relative_position_scores = (query_layer.unsqueeze(3) * positional_embedding.unsqueeze(0).unsqueeze(0)).sum(axis=-1)
                attention_scores = attention_scores + relative_position_scores
            elif self.position_embedding_type == "relative_key_query":
                # ops.einsum not support Static Graph
                ...
                relative_position_scores_query = (query_layer.unsqueeze(3) * positional_embedding.unsqueeze(0).unsqueeze(0)).sum(axis=-1)
                relative_position_scores_key = (key_layer.unsqueeze(2) * positional_embedding.unsqueeze(0).unsqueeze(0)).sum(axis=-1)
...
def BertLayer_jit(layer_module, 
                  hidden_states,
                    attention_mask,
                    layer_head_mask,
                    encoder_hidden_states,
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module( # layer_outputs为元组
                    hidden_states,
                    attention_mask,
                    layer_head_mask,
                    encoder_hidden_states,
            ...
            hidden_states = layer_outputs[0] # 只要最后的layer_output
            layer_outputs_tuple  +=  (layer_outputs, ) # 使用mtuple会报错。

        return hidden_states, layer_outputs_tuple
        

def encoder_getoutput(return_dict, 
                   hidden_states,
                    next_decoder_cache,
                    all_hidden_states,
                    all_self_attentions,
    ...
    return BaseModelOutputWithPastAndCrossAttentions( #hidden_states在JIT编译的时候显示nonconstant
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = BertEncoder(config).jit()
    ...
    def forward(
        ...
        # )
        hidden_states, layer_outputs_tuple = encoder_jit(self.encoder,
            embedding_output,
            attention_mask=extended_attention_mask,
            head_mask=head_mask,
            encoder_hidden_states=encoder_hidden_states,
        ...
        for layer_outputs in layer_outputs_tuple:
            if use_cache:
                next_decoder_cache += (layer_outputs[-1],) # 要每一层的倒数第一个layer_outputs, 最后叠加
        ...
        if not return_dict:
            encoder_outputs = tuple(v for v in [
                        hidden_states,
                        next_decoder_cache,
                        all_hidden_states,
                        all_self_attentions,
            ...
            encoder_outputs = BaseModelOutputWithPastAndCrossAttentions( #hidden_states在JIT编译的时候显示nonconstant
                            last_hidden_state=hidden_states,
                            past_key_values=next_decoder_cache,
                            hidden_states=all_hidden_states,
                            attentions=all_self_attentions,
