...
from mindspore.ops.operations  import Transpose, Reshape, Shape
from mindnlp.core.modules.flash_attention import FlashAttention
...
class MixtralRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        ...
        self.weight = nn.Parameter(mindspore.mint.ones(hidden_size))
    ...
    def forward(self, hidden_states):
        ...
        # variance = mindspore.mint.mean(hidden_states.pow(2), -1, keepdim=True)
...
class MixtralRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000):
        ...
        inv_freq = 1.0 / (self.base ** (mindspore.mint.arange(0, self.dim, 2, dtype=mindspore.int64).float() / self.dim))
    ...
    def _set_cos_sin_cache(self, seq_len, dtype):
        ...
        t = mindspore.mint.arange(self.max_seq_len_cached, dtype=mindspore.int64).type_as(self.inv_freq)
...
def rotate_half(x):
    ...
    x1, x2 = mindspore.mint.split(x, x.shape[-1] // 2, dim=-1)
...
class MixtralAttention(nn.Module):
    ...
    def __init__(self, config: MixtralConfig, layer_idx: Optional[int] = None):
        ...
        self.split_qkv = mindspore.ops.auto_generate.SplitWithSize()
        ...
        self.merger_head_transpose = Transpose()
    ...
    def forward(
        ...
        attn_weights = mindspore.mint.matmul(query_states, ops.transpose(key_states, 2, 3)) / math.sqrt(self.head_dim)
        ...
        # # 如果 attention_mask 不存在，生成一个新的 causal_mask
        # if attention_mask is None:
        #     causal_mask = ops.ones((q_len, kv_seq_len), mindspore.float32)
        #     causal_mask = ops.tril(causal_mask)  # 使用 tril 创建下三角矩阵
            
        #     # 将 causal_mask 转换成适合的形状以便在 FlashAttention 中使用
        #     causal_mask = ops.expand_dims(causal_mask, 0)  # 扩展 batch 维度
        #     causal_mask = causal_mask.expand_dims(0)  # 扩展 head 维度
        #     causal_mask = (1.0 - causal_mask) * (-1e9)  # 将非下三角部分设为大负值，以便在 softmax 中忽略

        # context_layer = self.flash_attention(query_states, key_states, value_states, causal_mask)
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: mindspore.Tensor) -> mindspore.Tensor:
        ...
        routing_weights /= mindspore.mint.sum(routing_weights, dim=-1, keepdim=True)
        ...
        for expert_idx in range(self.num_experts):
            ...
            idx, top_x = mindspore.mint.nonzero(expert_mask[expert_idx], as_tuple=True)
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                ...
                sequence_lengths = mindspore.mint.eq(input_ids, self.config.pad_token_id).int().argmax(-1) - 1
                sequence_lengths = sequence_lengths % input_ids.shape[-1]
            else:
                sequence_lengths = -1

        # pooled_logits = logits[ops.arange(batch_size), sequence_lengths]
        pooled_logits = logits[mindspore.mint.arange(batch_size), sequence_lengths]
