...
def generate_training_data(num_datasets=50):
    ...
    for i in range(num_datasets):
        ...
        # 生成距离矩阵（对称矩阵）
        distance_matrix = np.random.uniform(1, 40, (num_attractions, num_attractions))
...
def train_model(env, training_datasets, total_timesteps=500000):
    """训练模型"""
    print("开始训练模型...")
    model = PPO(
        "MlpPolicy",
        env,
        learning_rate=2.5e-4,  # 调整学习率
        n_steps=2048,
        batch_size=128,  # 增加批大小
        n_epochs=15,  # 增加训练轮数
        gamma=0.995,  # 调整折扣因子
        gae_lambda=0.98,
        clip_range=0.15,  # 调整裁剪范围
...
if __name__ == "__main__":
    ...
    # 生成训练数据
    print("生成训练数据...")
    training_datasets = generate_training_data(num_datasets=5000)
    
    # 训练模型
    model = train_model(env, training_datasets, total_timesteps=50000)
    
    # 保存模型
    model.save("flexible_tourist_model_10_20_min")
    print("模型已保存")
