...
class Informer(nn.Cell):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len, 
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(
                    AttentionLayer(ProbAttention(False, factor, attention_dropout=dropout, output_attention=output_attention, args=args),
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(
                    AttentionLayer(ProbAttention(True, factor, attention_dropout=dropout, output_attention=False, args=args), 
                                d_model, n_heads, mix=mix),
                    AttentionLayer(ProbAttention(False, factor, attention_dropout=dropout, output_attention=False, args=args), 
                                d_model, n_heads, mix=False),
                    d_model,
                    d_ff,
                    dropout=dropout,
