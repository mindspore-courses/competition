...
from utils.misc import as_list
...
def load_listed(dataset: Dataset, config: Dict[str, Any]):
    dataloader=GeneratorDataset(
        source=dataset,
        column_names=["camera_mono"]
        # shuffle=config['train']['shuffle']
    )
    # dataloader=dataloader.batch(batch_size=config['train']['batch_size'],drop_remainder=True, per_batch_map=listed_collating)
    return dataloader