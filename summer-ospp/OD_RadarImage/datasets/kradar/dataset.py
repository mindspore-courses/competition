...
from mindspore.dataset.vision import read_image


from datasets.kradar.utils import radar_info
...
class KRadarDataset(Dataset):
    ...
    def __getitem__(self, index) -> Dict[str, ms.Tensor]:
    ...
    def _add_transformations(self, sample: Dict[str, ms.Tensor]) -> Dict[str, ms.Tensor]:
        ...
        """
        if 'M' in self.camera:
            sample['label_to_camera_mono_t'] = ops.zeros_like(sample['label_to_camera_mono'])
        if 'S' in self.camera:
            sample['label_to_camera_stereo_t'] = ops.zeros_like(sample['label_to_camera_stereo'])
        if 'B' in self.radar:
    ...
    def _add_projections(self, sample: Dict[str, ms.Tensor]) -> Dict[str, ms.Tensor]:
    ...
    def _add_shape(self, sample: Dict[str, ms.Tensor]) -> Dict[str, ms.Tensor]:
        ...
        """
        if 'M' in self.camera:
            sample['camera_mono_shape'] = ms.Tensor(sample['camera_mono'].shape)
        if 'S' in self.camera:
            sample['camera_stereo_shape'] = ms.Tensor(sample['camera_stereo'].shape)
        if 'B' in self.radar:
            sample['radar_bev_shape'] = ms.Tensor(sample['radar_bev'].shape)
        if 'F' in self.radar:
            sample['radar_front_shape'] = ms.Tensor(sample['radar_front'].shape)

        return sample

    def _get_radar_ea_projection(self) -> ms.Tensor:
        ...
        return ms.Tensor([
            ...
            [0, 0, 0, 1]
        ]).type(getattr(ms, self.dtype))

    def _get_radar_ra_projection(self) -> ms.Tensor:
    ...
    def scale_radar_data(self, sample: Dict[str, ms.Tensor]) -> Dict[str, ms.Tensor]:
        ...
        for k, v in sample.items():
            if k in {'radar_bev', 'radar_front'}:
                ...
                sample[k] = ops.clip(sample[k], 0, 255)

        return sample

    def resize_image(self, sample: Dict[str, ms.Tensor]) -> Dict[str, ms.Tensor]:
        ...
        if 'M' in self.camera:
            sample['camera_mono'] = ms.Tensor(sample['camera_mono'], dtype=ms.float32)
            sample['camera_mono'] = sample['camera_mono'].movedim(-1, 0).expand_dims(0)  
            sample['camera_mono'] = ops.interpolate(sample['camera_mono'],size=(512, 910),mode='bilinear')
            sample['camera_mono'] = ops.squeeze(sample['camera_mono'], axis=0).movedim(0, -1)
        if 'S' in self.camera:
            sample['camera_stereo']=sample['camera_stereo'].movedim(-1,0).expand_dims(0)
            sample['camera_stereo'] = ops.interpolate(sample['camera_stereo'], size=(512,910))
            sample['camera_stereo']=ops.squeeze(sample['camera_stereo'],axis=0).movedim(0,-1)
            sample['camera_stereo']=ms.Tensor(sample['camera_stereo'],dtype=ms.float32)

        return sample

    def get_detection_label(self, raw_label: Dict[str, ms.Tensor]) -> Dict[str, ms.Tensor]:
        ...
        label['gt_angle'] = ops.cat(
            (ops.sin(raw_label[:, (3, )]), ops.cos(raw_label[:, (3, )])),
            axis=-1
        )

        # One hot encode class labels (+1 for ignore class)
        label['gt_class'] = ops.one_hot(
            raw_label[:, 7].long() + 1,
            self.num_classes
        ).type(getattr(ms, self.dtype))

        # Get configured field of view
        x_min, x_max = self.fov.get('x', ms.tensor([-float('inf'), float('inf')]))
        y_min, y_max = self.fov.get('y', ms.tensor([-float('inf'), float('inf')]))
        z_min, z_max = self.fov.get('z', ms.tensor([-float('inf'), float('inf')]))
        a_min, a_max = self.fov.get('azimuth', ms.tensor([-float('inf'), float('inf')]))

        # Get azimuth angle of the center points
        azimuth = ops.rad2deg(ops.arctan2(label['gt_center'][:, 1], label['gt_center'][:, 0]))
    ...
    def load_sample_data(self, sample_path: Dict[str, str]) -> Dict[str, ms.Tensor]:
        ...
        for key, path in sample_path.items():
            if osp.splitext(path)[-1] in {'.png', '.jpg'}:
                # Load image
                img: ms.Tensor = ms.Tensor(read_image(path),dtype=ms.float32)
                # Change to channel last format (C, H, W) -> (H, W, C)
                sample[key] = img

            if osp.splitext(path)[-1] in {'.npy'}:
                sample[key] = ms.from_numpy(np.load(path))
                sample[key] = ops.Cast()(sample[key], ms.float32)

        return sample

    def modality_dropout(self, sample: Dict[str, ms.Tensor]) -> Dict[str, ms.Tensor]:
        ...
        for modality in drawing:
            if modality in sample:
                sample[modality] = ms.zeros_like(sample[modality])
