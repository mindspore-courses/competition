...
def _non_zipping_check(batch_data: Mapping, detach: bool, pad: bool, fill_value):
...
def pickle_operations(data,key:str="_transforms",is_encode:bool=True):
    """
    Applied_operations are dictionaries with varying sizes, this medthod
    converts them to bytes so that we can (de-)collate.

    Args:
        data: a list of dictionary with substrcutures to be pickled/unpickled.
        key: the key suffixx for the target substrctures, default to "_transforms".
