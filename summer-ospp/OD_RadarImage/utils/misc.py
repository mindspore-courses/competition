...
def as_dtype(dtype:str):
    """Decrator to convert all inputs to a numpy 
    array of a given data type
...
def interp(x:ms.Tensor,
           xp: ms.Tensor,
           fp:ms.Tensor,
           left:float = None,
           right:float=None)-> ms.Tensor:
    """One-dimensional linear interpolation for monotonically increasing sample points.

    Returns the one-dimensional piecewise linear interpolant to a function
    with given discrete data points (xp,fp) evaluate at x.
    ...
    if ops.isclose(x1-x0,ops.zeros_like(x0)):
        y=ops.zeros_like(x)
    else:
        y=y0+(x-x0)*(y1-y0)/(x1-x0)
    
    # Adjust values outside of the origin value range
    y[x<x0]=left
    y[x>x0]=right
