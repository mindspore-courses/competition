from __future__ import annotations #noqa: F047
...
class SinusoidalEmbedding(nn.Cell):
    ...
    def construct(self, batch: ms.Tensor) -> ms.Tensor:
        ...
        dim_t = ms.ops.arange(self.num_feats, dtype=dtype)
...
class MultiLevelSinusoidalEmbedding(nn.Cell):
    def __init__(self,
                 n_levels: int = 1,
                 **kwargs):
        """
        ...
        self.embedding_layers = nn.CellDict({
            'embedding' + str(i):
    ...
    def construct(self, batches: OrderedDict[str, ms.Tensor]) -> OrderedDict[str, ms.Tensor]:
        """
        Arguments:
...
def build_sinusoidal_embedding(*args, **kwargs) -> nn.Cell:
