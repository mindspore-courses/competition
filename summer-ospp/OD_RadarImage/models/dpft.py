...
from models.queries import build_querent
from models.fusers import build_fuser
...
def _build_module(build_fn: Callable, module_name: str,
    ...
    if module is not None:
        return build_fn(
            module['name'], dict(module), *args, **kwargs
        )
...
class DPFT(nn.Cell):
    def __init__(self,
                 ...
                 backbones: Dict[str, nn.Cell] = None,
                 necks: Dict[str, nn.Cell] = None,
                 embeddings: Dict[str, nn.Cell] = None,
                 querent: nn.Cell = None,
                 fuser: nn.Cell = None,
                 head: nn.Cell = None,
                 **kwargs):
        ...
        self.skiplinks = {input: self.skiplinks.get(input, True) for input in inputs}
    ...
    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> DPFT:  # noqa: F821
    ...
    def _init_unspecified(self, submodule: Dict[str, nn.Cell]) -> Dict[str, nn.Cell]:
    ...
    @staticmethod
    def _add_raw_data(features: OrderedDict[str, ms.Tensor],
                      raw_data: ms.Tensor) -> OrderedDict[str, ms.Tensor]:
    ...
    def _get_projetions(inputs: List[str],
                        batch: Dict[str, ms.Tensor]) -> List[Tuple[ms.Tensor, ms.Tensor]]:
    ...
    def construct(self, batch: Dict[str, ms.Tensor]) -> OrderedDict[str, ms.Tensor]:
        ...
        # print(batch['camera_mono'][0,20:30,20:30,1])
        # print(batch['radar_bev'][0,20:30,20:30,1])
...
def build_dpft(*args, **kwargs):
    return DPFT.from_config(*args, **kwargs)