...
class KRadarExporter():
    def __init__(self,
        ...
        Arguments:
            conf_thrs: List of confidence thresholds to limit the export to.
            categories: Category mapping of the dataset classes.Maps a dataset class to a numerical category.
    ...
    def _construct_objects(self, objects: Dict[ms.Tensor],
                           conf_thr: float, pre: str = '') -> ms.Tensor:
        ...
        confidence, categories = ms.ops.max(objects[f"{pre}class"], dim=-1)

        # Calculate yaw angle from sin and cos part
        angle = ms.ops.atan2(objects[f'{pre}angle'][..., 0], objects[f'{pre}angle'][..., 1])
        ...
        return ms.ops.hstack([
            categories[mask].unsqueeze(-1),
            ms.ops.zeros((N, 1), dtype=float, device=device),
            ms.ops.zeros((N, 1), dtype=int, device=device),
            ms.ops.zeros((N, 1), dtype=int, device=device),
            ms.tensor([[50, 50, 150, 150]], dtype=int, device=device).repeat(N, 1),
            ms.ops.atleast_2d(objects[f"{pre}size"][mask, :][:, [2, 1, 0]]),
            ms.ops.atleast_2d(objects[f"{pre}center"][mask, :][:, [1, 2, 0]]),
            angle[mask].unsqueeze(-1)
        ])
    
    def _serialize_description(self, description: ms.Tensor) -> List[str]:
    ...
    def _serialize_object(self, object: ms.Tensor) -> str:
    ...
    def _export_output_objects(self, objects: Dict[ms.Tensor],
                               conf_thr: float, step: int,
                               description: ms.Tensor, dst: str) -> None:
    ...
    def _export_target_objects(self, objects: Dict[ms.Tensor],
                               conf_thr: float, step: int,
                               description: ms.Tensor, dst: str) -> None:
    ...
    def _export_output_batch(self, batch: Dict[str, ms.Tensor],
                             conf_thr: float, step: int,
        ...
        """
        # Decollate batch
        batch: List[Dict[str, ms.Tensor]] = decollate_batch(batch, detach=True, pad=False)

        for i, (data, desc) in enumerate(zip(batch, description)):
            self._export_output_objects(data, conf_thr, step + i, desc, dst)

    def _export_target_batch(self, batch: List[Dict[str, ms.Tensor]],
                             conf_thr: float, step: int, dst: str) -> List[ms.Tensor]:
    ...
    def export(self,
               outputs: Dict[str, ms.Tensor],
               targets: List[Dict[str, ms.Tensor]],
               step: int,
               dst: str) -> None:
