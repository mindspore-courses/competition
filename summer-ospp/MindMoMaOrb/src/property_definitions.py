...
def energy_row_fn(row: ase.db.row.AtomsRow, dataset: str) -> float:
    ...
    for row_attribute, conversion_factor in extract_info[dataset]:
        energy_ += get_property_from_row(row_attribute, row, conversion_factor)  # type: ignore
    return energy_
...
def forces_row_fn(row: ase.db.row.AtomsRow, dataset: str):
    ...
    forces_ = 0.0
    for row_attribute, conversion_factor in extract_info[dataset]:
        forces_ += get_property_from_row(row_attribute, row, conversion_factor)  # type: ignore
    return forces_
...
def stress_row_fn(row: ase.db.row.AtomsRow, dataset: str) -> float:
    ...
    stress_ = 0.0
    for row_attribute, conversion_factor in extract_info[dataset]:
        stress_ += get_property_from_row(row_attribute, row, conversion_factor)  # type: ignore
    return stress_
