...
class CAAttention(nn.Cell):
    def __init__(self, inp, oup, groups=32,act=True):
        super(CAAttention, self).__init__()
        mip = max(8, inp // groups)

        self.conv1 = nn.Conv2d(inp, mip, kernel_size=1, stride=1, padding=0, has_bias=True)
        self.bn1 = nn.BatchNorm2d(mip)
        self.conv2 = nn.Conv2d(mip, oup, kernel_size=1, stride=1, padding=0, has_bias=True)
        self.conv3 = nn.Conv2d(mip, oup, kernel_size=1, stride=1, padding=0, has_bias=True)
        self.relu =  SiLU() if act is True else (
    ...
    def construct(self, x):
        ...
        # 沿宽度方向求平均，结果维度为 (n, c, h, 1)
        x_h = self.mean(x, 3)
        # 沿高度方向求平均，结果维度为 (n, c, 1, w)
        x_w = self.mean(x, 2)
        ...
        x_h = self.sigmoid(self.conv2(x_h))
        x_w = self.sigmoid(self.conv3(x_w))

        # 扩展 x_h 和 x_w 以匹配输入的形状
        x_h = self.tile(x_h, (1, 1, 1, w))
        x_w = self.tile(x_w, (1, 1, h, 1))

        # 加权输入
        y = identity * x_h * x_w
        #print('99969966666666666666666666666666666666666666666666')   
        return y+identity

# if __name__ == '__main__':
#     from mindspore import context
#     context.set_context(mode=context.PYNATIVE_MODE)
#     x = Tensor(np.random.randn(1, 256, 64, 64), mindspore.float32)
#     coordatt = CAAttention(256, 256)
#     y = coordatt(x)
#     print("输入代码的shape为：", x.shape, "输出代码的shape为：", y.shape)
...
class BottleneckCSPWithCA(nn.Cell):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, shortcut=True, e=0.5):
        super(BottleneckCSPWithCA, self).__init__()
        print("BottleneckCSPWithCA........","c1===", c1, "c2===", c2)
        c_ = int(c2 * e)  # hidden channels
        self.conv1 = Conv(c1, c_, 1, 1)
        self.conv2 = Conv(c1, c_, 1, 1)
        self.conv3 = Conv(2 * c_, c2, 1)  # act=FReLU(c2)
        self.m = nn.SequentialCell(
            [Bottleneck(c_, c_, shortcut, e=1.0) for _ in range(n)])
        self.concat = ops.Concat(axis=1)
        self.coordatt=CAAttention(c1,c2)
    ...
    def construct(self, x):
        ...
        c4 = self.concat((c2, c3))
        c5 = self.conv3(c4)
        #print("输入代码的shape为：", x.shape, "输出代码的shape为：", c5.shape)
        # c6=self.coordatt(c5)
       # print("dalaohu  dalaohu")
       # c6=self.coordatt(c5)
        return c6
