...
class LVISMetric(CocoMetric):
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            result['img_id'] = data_sample['img_metas']['img_id']
            result['bboxes'] = pred['bboxes'].asnumpy()
            result['scores'] = pred['scores'].asnumpy()
            result['labels'] = pred['labels'].asnumpy()

            # parse gt
            gt = dict()
            gt['width'] = data_sample['img_metas']['ori_shape'][1]
            gt['height'] = data_sample['img_metas']['ori_shape'][0]
            gt['img_id'] = data_sample['img_metas']['img_id']
            if self._lvis_api is None:
                # TODO: Need to refactor to support LoadAnnotations
                assert 'instances' in data_sample, \
                    'ground truth is required for evaluation when ' \
                    '`ann_file` is not provided'
                gt['anns'] = data_sample['img_metas']['instances']
