    ...
    def __call__(self, results: dict) -> dict:
        ...
        if 'img' in results:
            ...
            if not img.flags.c_contiguous:
                ...
                img = ms.Tensor(img).permute(2, 0, 1).contiguous()
        ...
        for key in self.mapping_table.keys():
            ...
            if key == 'gt_masks' or 'box' in key:
                if 'gt_ignore_flags' in results:
                    ...
                    if isinstance(results[key], ms.Tensor):
                        instance_data[self.mapping_table[key]] = results[key][ms.Tensor(valid_idx)]
                        ignore_instance_data[self.mapping_table[key]] = results[key][ms.Tensor(ignore_idx)]
                ...
                if 'gt_ignore_flags' in results:
                    instance_data[self.mapping_table[key]] = ms.Tensor(results[key][valid_idx])
                    ignore_instance_data[self.mapping_table[key]] = ms.Tensor(results[key][ignore_idx])
                else:
                    instance_data[self.mapping_table[key]] = ms.Tensor(results[key])
        data_sample['gt_instances'] = instance_data
        data_sample['ignored_instances'] = ignore_instance_data

        if 'proposals' in results:
            data_sample['proposals'] = dict(
                bboxes=ms.Tensor(results['proposals']), scores=ms.Tensor(results['proposals_scores']))

        if 'gt_seg_map' in results:
            gt_sem_seg_data = dict(sem_seg=ms.Tensor(results['gt_seg_map'][None, ...].copy()))
