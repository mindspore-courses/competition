...
from .yolo_world_head import YOLOWorldHeadModule, YOLOWorldHead
...
def build_yoloworld_data_preprocessor(size: str, args: Optional[dict] = None) -> nn.Cell:
...
def build_yolov8_backbone(size: str, args: Optional[dict] = None) -> nn.Cell:
    """Exp.
    >>> model = build_yolov8_backbone('s')
    """
    cfg = load_config(size, YOLOv8BackboneCfg, 'yolov8_backbone', args)
    
    ms_yolov8_backbone = YOLOv8CSPDarknet(
                        arch=cfg.arch,
...
def build_yoloworld_text(size: str, args: Optional[dict] = None) -> nn.Cell:
    cfg = load_config(size, YOLOWorldTextCfg, 'yoloworld_text', args)

    ms_text = HuggingCLIPLanguageBackbone(
...
def build_yoloworld_backbone(
        size: str,
        image_model: nn.Cell,  # from `build_yolov8_backbone`
        text_model: nn.Cell,  # from `build_yoloworld_text`
        args: Optional[dict] = None) -> nn.Cell:
    cfg = load_config(size, YOLOWorldBackboneCfg, 'yoloworld_backbone', args)
    
    ms_yoloworld_backbone = MultiModalYOLOBackbone(
...
def build_yoloworld_neck(size: str, args: Optional[dict] = None) -> nn.Cell:
...
def build_yoloworld_head(
    ...
    ms_head_module = YOLOWorldHeadModule(
...
def build_yoloworld_detector(size: str,
                                ...
                                neck: nn.Cell,
                                bbox_head: nn.Cell,
                                data_preprocessor: Optional[nn.Cell] = None,
                                args: Optional[dict] = None) -> nn.Cell:
    cfg = load_config(size, YOLOWorldDetectorCfg, 'yoloworld_detector', args)
    ms_detector = YOLOWorldDetector(
        backbone=backbone,
        neck=neck,
        bbox_head=bbox_head,
        mm_neck=cfg.mm_neck,
