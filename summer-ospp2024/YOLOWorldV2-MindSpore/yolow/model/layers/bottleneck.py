...
class SPPFBottleneck(nn.Cell):
    ...
    def __init__(self,
        ...
        if isinstance(kernel_sizes, int):
            self.poolings = nn.MaxPool2d(kernel_size=kernel_sizes, stride=1, pad_mode="pad", padding=kernel_sizes // 2)
            conv2_in_channels = mid_channels * 4
        else:
            self.poolings = nn.CellList(
                [nn.MaxPool2d(kernel_size=ks, stride=1, pad_mode="pad", padding=ks // 2) for ks in kernel_sizes])
    ...
    def construct(self, x: Tensor) -> Tensor:
        ...
        if isinstance(self.kernel_sizes, int):
            y1 = self.poolings(x)
            y2 = self.poolings(y1)
            x = ms.ops.cat([x, y1, y2, self.poolings(y2)], axis=1)
        else:
            x = ms.ops.cat([x] + [pooling(x) for pooling in self.poolings], axis=1)
        x = self.conv2(x)
        return x
