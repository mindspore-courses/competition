...
class MlvlPointGenerator:
    ...
    def _meshgrid(self, x: Tensor, y: Tensor, row_major: bool = True) -> Tuple[Tensor, Tensor]:
        ...
        yy, xx = ms.ops.meshgrid(y, x, indexing='ij')
        if row_major:
    ...
    def single_level_grid_priors(self,
        ...
        shift_x = (ms.ops.arange(0, feat_w) + self.offset) * stride_w
        # keep featmap_size as Tensor instead of int, so that we
        # can convert to ONNX correctly
        shift_x = shift_x.to(dtype)

        shift_y = (ms.ops.arange(0, feat_h) + self.offset) * stride_h
        # keep featmap_size as Tensor instead of int, so that we
        # can convert to ONNX correctly
        shift_y = shift_y.to(dtype)
        shift_xx, shift_yy = self._meshgrid(shift_x, shift_y)
        if not with_stride:
            shifts = ms.ops.stack([shift_xx, shift_yy], axis=-1)
        else:
            ...
            shifts = ms.ops.stack([shift_xx, shift_yy, stride_w, stride_h], axis=-1)
        # all_points = shifts.to(device)
        return shifts

    def valid_flags(self,
                    featmap_sizes: List[Tuple[int, int]],
                    pad_shape: Tuple[int],) -> List[Tensor]:
        ...
        for i in range(self.num_levels):
            ...
            flags = self.single_level_valid_flags((feat_h, feat_w), (valid_feat_h, valid_feat_w))
            multi_level_flags.append(flags)
        return multi_level_flags

    def single_level_valid_flags(self,
                                 featmap_size: Tuple[int, int],
                                 valid_size: Tuple[int, int],) -> Tensor:
        ...
        valid_x = ms.ops.zeros(feat_w, dtype=ms.bool_)
        valid_y = ms.ops.zeros(feat_h, dtype=ms.bool_)
        valid_x[:valid_w] = 1
        valid_y[:valid_h] = 1
    ...
    def sparse_priors(self,
                      ...
                      featmap_size: Tuple[int],
                      level_idx: int,
                      dtype: ms.dtype = ms.float32,) -> Tensor:
        ...
        prioris = ms.ops.stack([x, y], axis=1).to(dtype)
        return prioris
