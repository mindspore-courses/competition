...
class YOLOWDetDataPreprocessor(nn.Cell):
    ...
    def __init__(self,
        ...
        if mean is not None:
            ...
            self.mean = ms.Parameter(ms.Tensor(mean).view(-1, 1, 1))
            self.std = ms.Parameter(ms.Tensor(std).view(-1, 1, 1))
        else:
    ...
    def construct(self, data: dict, training: bool = False) -> dict:
        ...
        pad_h = target_h - h
        pad_w = target_w - w
        inputs = ms.ops.pad(inputs, (0, pad_w, 0, pad_h), 'constant', self.pad_value)

        assert tuple(inputs[0].shape[-2:]) == tuple(inputs.shape[2:])  # debug
        if not training:
    ...
    def _get_pad_shape(self, _batch_inputs: ms.Tensor) -> List[tuple]:
