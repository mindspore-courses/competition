...
class YOLOWorldPAFPN(nn.Cell):
    ...
    def __init__(self,
        ...
        self.top_down_layers = nn.CellList()
        ...
        self.bottom_up_layers = nn.CellList()
    ...
    def build_reduce_layer(self, idx: int) -> nn.Cell:
        return nn.Identity()

    def build_upsample_layer(self, *args, **kwargs) -> nn.Cell:
        return nn.Upsample(scale_factor=2.0, mode='nearest', recompute_scale_factor=True)

    def build_downsample_layer(self, idx: int) -> nn.Cell:
        return Conv(
    ...
    def build_out_layer(self, *args, **kwargs) -> nn.Cell:
        return nn.Identity()

    def build_top_down_layer(self, idx: int) -> nn.Cell:
    ...
    def build_bottom_up_layer(self, idx: int) -> nn.Cell:
        return MaxSigmoidCSPLayer(
    ...
    def construct(self, img_feats: List[Tensor], txt_feats: Tensor) -> tuple:
        """Forward function.
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            if self.upsample_feats_cat_first:
                top_down_layer_inputs = ms.ops.cat([upsample_feat, feat_low], 1)
            else:
                top_down_layer_inputs = ms.ops.cat([feat_low, upsample_feat], 1)
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            out = self.bottom_up_layers[idx](ms.ops.cat([downsample_feat, feat_high], 1), txt_feats)
            outs.append(out)
